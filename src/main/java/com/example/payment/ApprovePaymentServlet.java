...
public class ApprovePaymentServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            ...
            try (BufferedReader reader = new BufferedReader(new FileReader(paymentHistoryFile))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (paymentDetails.length >= 6 && paymentDetails[0].equals(orderId)) {
                        found = true;
                        paymentDetails[paymentDetails.length - 1] = "Approved"; // Update status to Approved
        ...
        if (success) {
            ...
            response.sendRedirect(request.getContextPath() + "/AdminPage/AdminDashboard.jsp?success=paymentApproved");
        } else {
